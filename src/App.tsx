...
import { ArrowUpRight, ArrowLeft, ArrowRight, Leaf, MoonStars, Sun, SlidersHorizontal, X, ArrowCounterClockwise, Play, Pause, BookOpen, Cube, Check } from '@phosphor-icons/react'
...
export default function App() {
  ...
  return <main className={`town-app ${env.daylight > 1 ? 'is-day' : ''} ${cinema ? 'is-cinema' : ''}`} style={{ '--sky-tint': env.tint, '--backdrop-brightness': env.backdrop } as CSSProperties}>
    ...
    <section className="introduction" aria-label="欢迎来到 Will 的小镇"><p className="eyebrow"><span className="status-dot" /> 一个独立开发者的数字住处</p><h1>小镇的灯亮着，<br />新的<span>想法</span><br className="mid-break" />也在生长。</h1><p className="intro-copy">我是 Will，一名独立开发者。<br />这里的每家小店，都放着我做的产品。</p><div className="intro-actions"><button className="primary-button" onClick={() => onOpen('products')}>查看我的产品<ArrowRight size={20} /></button><button className="text-button" onClick={() => onOpen('journal')}>读读开发手记 <ArrowUpRight size={16} /></button></div><div className="tiny-note"><Cube size={14} /> 一条小街，一些正在发生的事。</div></section>
    <div ref={region} className="world-region" tabIndex={-1} role="region" aria-label="Will 的小镇，点击店招查看产品与故事。" data-ready={ready} data-phase={env.phase}><SceneBoundary><Suspense fallback={null}><World minutes={minutes} mobile={mobile} onOpen={onOpen} onReady={onReady} /></Suspense></SceneBoundary></div>
