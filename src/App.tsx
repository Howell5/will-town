...
import { ArrowUpRight, ArrowLeft, ArrowRight, ArrowUp, ArrowDown, HouseLine, MoonStars, Sun, SlidersHorizontal, X, ArrowCounterClockwise, Play, Pause, BookOpen, Cube, Check } from '@phosphor-icons/react'
...
export default function App() {
  ...
  return <main className={`town-app ${env.daylight > 1 ? 'is-day' : ''} ${exploring ? 'is-exploring' : ''} ${cinema ? 'is-cinema' : ''}`} style={{ '--sky-tint': env.tint, '--backdrop-brightness': env.backdrop } as CSSProperties}>
    ...
    <div className="backdrop" aria-hidden="true"><img src="/assets/backgrounds/distant-town.png" alt="" /><div className="sky-tint" /></div>
    <header className="header"><button className="brand" onClick={reset} aria-label="回到小镇首页"><HouseLine size={25} weight="duotone" /><span>WILL <span className="brand-cn">的小镇</span></span><span className="edition">VOL. 001</span></button><nav id="content-navigation" aria-label="主导航"><button onClick={() => onOpen('products')}>产品<span className="nav-dot" /></button><button onClick={() => onOpen('journal')}>开发手记</button><button onClick={() => onOpen('studio')}>关于我 <ArrowUpRight size={14} /></button></nav></header>
    <section className="introduction" aria-label="欢迎来到 Will 的小镇"><p className="eyebrow"><span className="status-dot" /> 一个独立开发者的数字住处</p><h1>小镇的灯亮着，<br />新的<span>想法</span><br className="mid-break" />也在生长。</h1><p className="intro-copy">我是 Will，一名独立开发者。<br />这里的每家小店，都放着我做的东西。</p><div className="intro-actions"><button className="primary-button" onClick={onExplore} disabled={!ready}>{ready ? '来小镇逛逛' : '正在布置小镇…'}<ArrowRight size={20} /></button><button className="text-button" onClick={() => onOpen('products')}>直接查看产品 <ArrowUpRight size={16} /></button></div><div className="tiny-note"><Cube size={14} /> 一条小街，一些正在发生的事。</div></section>
    <div ref={region} className="world-region" tabIndex={0} role="region" aria-label="可探索的小镇。方向键或 WASD 移动，靠近店铺按 E 进入。也可直接点击店招。" data-ready={ready} data-phase={env.phase}><SceneBoundary><Suspense fallback={null}><World minutes={minutes} runtime={runtime} reduced={reduced} mobile={mobile} onOpen={onOpen} onNear={setNear} onExplore={onExplore} onReady={onReady} region={region} /></Suspense></SceneBoundary></div>
